...
public:
	...
	Camera(vec3 position = vec3(0.0f, 0.7f, 3.5f), vec3 up = vec3(0.0f, 1.0f, 0.0f), GLfloat yaw = YAW, GLfloat pitch = PITCH) 
		: Front(vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVTY), Zoom(ZOOM)
	{
		...
		this->Front = -position;
		this->Pitch = degrees(-atanf(position.y / position.z));;
	}
	...
	void shake(GLfloat xoffset, GLfloat yoffset, GLboolean constrainPitch = true)
	{
